...
func main() {
	...
	go func() {
		if err := consumer.Consume(ctx, messaging.QueueCatalogAccessCommands, func(msgCtx context.Context, env messaging.Envelope) error {
			switch env.Type {
			case messaging.RoutingKeyAccessGrantRequested:
				return amqpHandler.HandleAccessGrantRequested(msgCtx, env)
			case messaging.RoutingKeyAccessRevokeRequested:
				return amqpHandler.HandleAccessRevokeRequested(msgCtx, env)
			default:
